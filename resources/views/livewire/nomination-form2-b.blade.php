                ...
                <div class="modal-dialog modal-xl modal-dialog-scrollable">
                    <div class="modal-content p-4">
                        ...
                        <div class="d-flex justify-content-between align-items-center mb-3">
                            <h5 class="mb-0">Nomination Form – Preview</h5>
                            <button class="btn-close" wire:click="closePreview"></button>
                        </div>

                        <div style="width:114px;height:145px;border:1px solid #000;display:flex;align-items:center;justify-content:center;">
                            @if($data->candidate_photo)
                                <img src="{{ public_path('storage/'.$data->candidate_photo) }}"
                        ...
                        <h6 class="text-center fw-bold">PART I</h6>

                        <p>
                            I nominate as a candidate for election to the Legislative Assembly from the
                            <strong>{{ $assembly_name }}</strong> Assembly Constituency.
                        </p>

                        <p>
                            Candidate's name:
                            <strong>{{ $candidate_name }}</strong>,
                            {{ ucfirst($relation_type) }}’s name:
                        ...
                        <p>
                            Electoral Roll:
                            Sl. No <strong>{{ $candidate_sl_no }}</strong>,
                            Part No <strong>{{ $candidate_part_no }}</strong>
                        </p>

                        <hr>

                        <h6 class="text-center fw-bold">PART III</h6>

                        <p>Age: <strong>{{ $candidate_age }}</strong> years</p>

                        <p>
                            Party Name:
                            <strong>{{ $party_name }}</strong>
                            ({{ ucfirst($party_type) }} Party)
                        </p>

                        <p>
                            Election Type:
                            <strong>{{ ucfirst($election_type) }}</strong>
                        </p>

                        <hr>

                        <h6 class="text-center fw-bold">PART IIIA</h6>

                        <p>
                            Convicted:
                            <strong>{{ strtoupper($convicted) }}</strong>
                        </p>

                        @if($convicted === 'yes')
                            <p>Case No: <strong>{{ $case_no }}</strong></p>
                            <p>Police Station: <strong>{{ $police_station }}</strong></p>
                            <p>District: <strong>{{ $district }}</strong></p>
                            <p>State: <strong>{{ $state }}</strong></p>
                            <p>Sections: <strong>{{ $sections }}</strong></p>
                            <p>Conviction Date: <strong>{{ $conviction_date }}</strong></p>
                            <p>Court: <strong>{{ $court }}</strong></p>
                            <p>Punishment: <strong>{{ $punishment }}</strong></p>
                        @endif
                        ...
                        <p>
                            Office of Profit:
                            <strong>{{ strtoupper($office_of_profit) }}</strong>
                        </p>
                        @if($office_of_profit === 'yes')
                            <p>Details: <strong>{{ $office_details }}</strong></p>
                        @endif

                        <p>
                            Insolvent:
                            <strong>{{ strtoupper($insolvent) }}</strong>
                        </p>

                        <p>
                            Foreign Allegiance:
                            <strong>{{ strtoupper($foreign_allegiance) }}</strong>
                        </p>

                        <p>
                            Government Contract:
                            <strong>{{ strtoupper($govt_contract) }}</strong>
                        </p>

                        <p>
                            Company Position:
                            <strong>{{ strtoupper($company_position) }}</strong>
                        </p>

                        <p>
                            Commission Disqualified:
                            <strong>{{ strtoupper($commission_disqualified) }}</strong>
                        </p>

                        <hr>

                        <div class="text-end">
                            <button class="btn btn-secondary" wire:click="closePreview">
                                Back to Edit
                            </button>
                        </div>
