<div>
                ...
                <div class="col-md-3" wire:ignore>
                    ...
                    <select wire:model="assembly" class="form-select chosen-select">
                        ...
                        @foreach($allAssemblies as $assembly_item)
                            <option value="{{ $assembly_item->id }}">({{ $assembly_item->assembly_code }}) {{ $assembly_item->assembly_name_en }}</option>
                        @endforeach
                    </select>
                </div>
                    ...
                    <select wire:model="campaignAssembly" class="form-select chosen-select">
                        ...
                        @foreach($allAssemblies as $assembly_item)
                            <option value="{{ $assembly_item->id }}">
                                ({{ $assembly_item->assembly_code }}) {{ $assembly_item->assembly_name_en }}
                            </option>
                        @endforeach
                    </select>
                </div>
    ...
    <div class="loader-container" wire:loading wire:target="ChangeCampaignField">
        <div class="loader"></div>
    </div>
    @push('scripts')
    ...
    <script>
        function reinitChosen() {
            ...
            .on('change', function (e) {
                let model = $(this).attr('wire:model');
                if (model) {
                    @this.call('ChangeCampaignField', model, $(this).val());
                }
            });
...
class AdminDashboard extends Component
{
    ...
    public $pendingCampaigns = 0;
    public $approvedCampaigns = 0;
    ...
    public function mount(){
        ...
        $this->CampaignPermissionStatusField();
    ...
    public function ChangeCampaignField($key, $value)
    {
        $this->$key = $value;

        if ($key === 'campaignDistrict') {
            $this->campaignAssembly = '';
            $this->campaignEventCategory = '';

        } elseif ($key === 'campaignAssembly') {
            $this->campaignDistrict = '';
            $this->campaignEventCategory = '';

        } elseif ($key === 'campaignEventCategory') {
            $this->campaignDistrict = '';
            $this->campaignAssembly = '';
        }
    ...
    public function CampaignPermissionStatusField()
    {
        $baseQuery = Campaign::query();
        if ($this->campaignAssembly) {
            $baseQuery->where('assembly_id', $this->campaignAssembly);

        } elseif ($this->campaignDistrict) {

            $baseQuery->whereHas('assembly', function ($q) {
                $q->where('district_id', $this->campaignDistrict);
            });

        } elseif ($this->campaignEventCategory) {

            $baseQuery->where('event_category_id', $this->campaignEventCategory);
        }

        $this->totalCampaigns = (clone $baseQuery)->count();

        $this->pendingCampaigns = (clone $baseQuery)
            ->where('status', 'Pending')
            ->count();

        $this->approvedCampaigns = (clone $baseQuery)
    ...
    public function resetFilters(){ 
        $this->reset(['district','phase','assembly','campaignDistrict', 'campaignAssembly', 'campaignEventCategory']);
    }
